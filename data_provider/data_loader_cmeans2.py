...
def get_aug_data_v2(data_x, l):
    aug_x = data_x
    aug_method = [aug.jitter, aug.time_warp, aug.magnitude_warp, aug.window_warp, aug.window_slice]
    for _ in range(l):
        select_method = random.randint(0,len(aug_method)-1)
        aug_x_ = aug_method[select_method](data_x)
        aug_x = np.concatenate([aug_x, aug_x_], axis=-1)
    return aug_x
...
def get_clustr_data_v2(args, data, stamp, centers):
    ...
    if centers is None:
        ...
        overlapping_clusters = [
            np.where(membership[i] > train_threshold)[0] for i in range(len(data_x))
        ]
        pred_y_index = 0
        for sample in range(num_samples):
            for feature in range(num_features):
                ...
                for closest_cluster_index in overlapping_clusters[pred_y_index]:
...
class Dataset_ClusTR(Dataset):
    ...
    def generate_clustr_data(self):
        ...
        for set_type in range(2):
            ...
            if set_type == 0:
                sub_data = get_aug_data_v2(sub_data[np.newaxis,:], self.args.cluster_amount)    
                sub_data = sub_data.reshape((sub_data.shape[1], sub_data.shape[2]))
            ...
            for clustr_index in range(self.args.cluster_amount+1):
                if set_type == 0:
                    ...
                    if len(clustr_data_x[clustr_index]) > num_samples:
                        clustr_data_x[clustr_index] = clustr_data_x[clustr_index][::(len(clustr_data_x[clustr_index])//num_samples+1),:,:]
                        clustr_data_y[clustr_index] = clustr_data_y[clustr_index][::(len(clustr_data_x[clustr_index])//num_samples+1),:,:]
                        clustr_data_x_mark[clustr_index] = clustr_data_x_mark[clustr_index][::(len(clustr_data_x[clustr_index])//num_samples+1),:,:]
                        clustr_data_y_mark[clustr_index] = clustr_data_y_mark[clustr_index][::(len(clustr_data_x[clustr_index])//num_samples+1),:,:]
