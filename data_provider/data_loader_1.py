...
def get_clustr_data(args, data, stamp, centers):
...
class Dataset_ClusTR(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        self.data = df_data.values.reshape(df_data.values.shape[0] * df_data.values.shape[1], -1)
        if self.scale:
            train_data_samples = df_data[self.border1s[0]:self.border2s[0]].values.shape[0]
            train_data_features = df_data[self.border1s[0]:self.border2s[0]].values.shape[1]
            train_data = df_data[self.border1s[0]:self.border2s[0]].values.reshape(train_data_samples*train_data_features, -1)
            self.scaler.fit(train_data)
            self.data = self.scaler.transform(self.data)
        else:
            self.data = df_data.values
        self.data = self.data.reshape(df_data.values.shape[0], df_data.values.shape[1])
            ...
            for clustr_index in range(self.args.cluster_amount):
                if set_type == 0:
                    if 'ETT' in self.data_name:
                        ...
                        train_vali_rate = 7 / 1
                    split = int(train_vali_rate*len(clustr_data_x[clustr_index])/(train_vali_rate+1))
                    np.save(f'{self.clustr_data_path}/{clustr_index}_train_x.npy',clustr_data_x[clustr_index][:split])
